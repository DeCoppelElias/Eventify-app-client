...
export default function TagBox(tag, handleDelete){
    return (
        <div className='bg-gray-900 text-white text-center pl-2 pr-2 pt-1 pb-1 rounded-lg flex w-full h-full justify-center items-center'>
            <p >{String(tag)}</p>
            <button className='w-4 h-ful ml-1' onClick={() => handleDelete(tag)}>
                <svg className='w-full hover:stroke-white stroke-gray-700' viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
